...
-- Learned values from a handful of disenchants swing a lot (one shard vs. dust),
-- which made items flip between skipped and not after every cast. Only trust
-- them once the item's own bracket has enough samples.
local MIN_LEARNED_SAMPLES = 10

-- Expected disenchant value in copper: learned value if well sampled, else era table.
local function ExpectedValue(link)
	if ns.GetDisenchantValue then
		local actual, samples, approx, partial = ns.GetDisenchantValue(link)
		if actual and actual > 0 and not approx and not partial and samples >= MIN_LEARNED_SAMPLES then
			return actual
		end
	end
